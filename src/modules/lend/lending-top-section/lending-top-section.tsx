...
const TopSectionChart = dynamic(() => import('@/modules/lend/chart/top-section-chart'), {
  ssr: false,
});
...
const LendingTopSection = () => {
  ...
  const currentAPR = selectedMarketData?.netEarnAPY || 0;
  ...
  useEffect(() => {
    if (!selectedMarketData) {
      const neededData = pathname.replace('/lend/', '').split('-');

      const currentAsset = neededData[0];
      const currentNetwork = neededData[1];
...
export default LendingTopSection;
